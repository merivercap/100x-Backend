...
class ReplyService {
  ...
  fetchRepliesFromSteemit({ usersSteemitName, permLink }) {
    const addRepliesToDb = replies => {
      for (const reply of replies[0]) {
        // replies returned in 2-D array.  Our client.sendAsync supports multiple requests..
  ...
  findOrCreateReply(steemitReply, commenter) {
    return ReplyModel.findOrCreate({
      where: { id: steemitReply.id },
      defaults: {
        ...this.replyProperFormat(steemitReply),
        userId: commenter.id
      }
  ...
  replyProperFormat(steemitReply) {
    const convertedValue = Number.parseFloat(
      steemitReply.pending_payout_value.split("SBD")[0]
    );
    return {
      postId: this.post.id,
